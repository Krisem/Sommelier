...
- ⚠️ **Rate-limiten er nå det bindende planleggingsgrunnlaget — men det er TO bøtter, ikke én.** Søke-API-et (`/vmpws/`) svarer `Retry-After: 3399` (timeskvote, ~800–900 kall). **Produktside-HTML er en egen bøtte med `Retry-After: 300`** — fem minutter, målt 2026-08-29 etter 71 sider. De to straffes altså helt ulikt, og et estimat som blander dem er verdiløst. Kapasiteten per produktside-syklus er **under måling** (de 71 ble hentet mens søkekvoten allerede var tungt brukt, så tallet kan være kunstig lavt). Konklusjonen står uansett: dybde for alle 13 775 er i størrelsesorden 17 timer, så details er permanent et prioritert utsnitt — men begrunnelsen «16 kvotevinduer à én time» var bygget på feil bøtte og er rettet.
