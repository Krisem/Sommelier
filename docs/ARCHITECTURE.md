...
> **Amendment 2026-08-31.** Dekningstallet over gjelder katalogen slik den var da avsnittet ble skrevet. Målt mot HEAD `bbe9840` og `catalog.ndjson` med `count` 27 402: `v0.json` har **409 varenumre = 1,5 %**. Konklusjonen står uendret og er nå gjennomført — CLAUDE.md steg 6b peker på `classify_code`, med eksplisitt «ikke slå opp `v0.json`». Poenget med å føre amendmentet i stedet for å rette tallet: et dekningstall uten revisjonsmarkør er en påstand, ikke en måling, og tre forskjellige tall (0,47 % · 0,59 % · 1,5 %) har vært i omløp for samme fil fordi både teller og nevner har flyttet seg. Se `tasks/lessons.md` 2026-08-31.
