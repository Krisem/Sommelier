...
- **⚠️ SØK har OGSÅ en kort burst-bøtte, ikke bare timeskvoten (målt 2026-08-30).** Runbooken beskrev lenge søk som én ren timeskvote (`Retry-After: 3399`). Det stemmer bare ved lav takt. Ved høy takt slår en kortere bøtte inn først: **`Retry-After: 282` etter 78 vellykkede søkekall på ~2 minutter** (~40 kall/min). Rødvins-sveipen kjørte 520 sider på 39 min = ~13/min og traff den aldri — den gikk rett i timeskvoten. Modellen er altså tredelt: **~15 kall/min vedvarende → ellers ~78 kall så ~7,5 min pause → og timeskvoten på ~850/t over det igjen.** Praktisk: hold søketakten under ~15/min på lange jobber, ellers betaler du en burst-pause du ikke hadde budsjettert med.
